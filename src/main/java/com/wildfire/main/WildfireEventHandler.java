...
public class WildfireEventHandler {
    ...
    public void onPlayerAttack(AttackEntityEvent event) {
        ...
        BreastPhysics leftBreastPhysics = new BreastPhysics();
        BreastPhysics rightBreastPhysics = new BreastPhysics();
        leftBreastPhysics.applyAttackImpulse(player, settings.breastSize, settings.bounceMultiplier * settings.intensity / 100.0F, settings.momentum / 100.0F);
        if (!settings.breastsUniboob) {
            rightBreastPhysics.applyAttackImpulse(player, settings.breastSize, settings.bounceMultiplier * settings.intensity / 100.0F, settings.momentum / 100.0F);
        }
    }
}