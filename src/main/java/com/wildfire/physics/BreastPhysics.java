...
public class BreastPhysics {
    ...
    public void update(EntityLivingBase entity, float bustSize, float bounceMultiplier) {
        ...
        this.velocity = MathHelper.clamp_float(this.velocity + (this.targetBounceY - this.bounceVel) * delta, -5f, 5f);
        this.bounceVel += this.velocity * percent * 1.1625f;

        this.velocityX = MathHelper.clamp_float(this.velocityX + (this.targetBounceX - this.bounceVelX) * delta, -5f, 5f);
        this.bounceVelX += this.velocityX * percent;

        this.rotVelocity = MathHelper.clamp_float(this.rotVelocity + (this.targetRotVel - this.bounceRotVel) * delta, -5f, 5f);
    ...
    private float calcRotation(EntityLivingBase entity, float bounceIntensity) {
        return -((entity.rotationYawHead - entity.prevRotationYawHead) / 15f) * bounceIntensity;
    }
